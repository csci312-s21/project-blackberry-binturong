...
export const getTimeString = (hour, duration) => {
  ...
  return `${start.format("h:mm")} - ${start
    .add(duration, "hour")
    .format("h:mm a")}`;
};
...
// returns whether show is currently playing
export const showIsCurrent = (show) => {
  return show.hour === moment().hour() * 100;
};
...
export const getCurrentPlaylist = async () => {
  ...
  console.assert(
    currentPlaylists.length === 1,
    "multiple playlists are current"
  );
