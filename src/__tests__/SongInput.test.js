...
describe("SongInput tests", () => {
  ...
  test("Correct image appears when a valid album is entered", async () => {
    fetchMock.get(`${process.env.LASTFM_ROOT}?method=album.getinfo&api_key=${process.env.LASTFM_KEY}&artist=${song.artist}&album=${song.album}&format=json`, () => goodResponse);
